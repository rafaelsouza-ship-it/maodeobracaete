...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [availableUsers, setAvailableUsers] = useState<User[]>([
    { id: '1', name: 'Rafael Souza', email: 'rafael.souza@taua.com.br', role: UserRole.ADMIN },
    { id: '2', name: 'Vanessa Monteiro', email: 'vanessa.monteiro@taua.com.br', role: UserRole.ADMIN },
    { id: '3', name: 'Gestor Operacional', email: 'gestor@taua.com.br', role: UserRole.USER },
  ]);
  ...
  const checkAdminRole = async (email: string): Promise<UserRole> => {
    ...
    return data ? UserRole.ADMIN : UserRole.USER;
  };
  ...
  useEffect(() => {
    const recoverSession = async () => {
      ...
      if (session?.user) {
        const role = await checkAdminRole(session.user.email!);
        setUser({
          id: session.user.id,
          email: session.user.email!,
          name: session.user.email!.split('@')[0].replace('.', ' ').replace(/\b\w/g, l => l.toUpperCase()),
          role
        });
  ...
  const login = async (email: string, pass: string): Promise<boolean> => {
    ...
    if (data.user) {
      const role = await checkAdminRole(data.user.email!);
      setUser({
        id: data.user.id,
        email: data.user.email!,
        name: data.user.email!.split('@')[0].replace('.', ' ').replace(/\b\w/g, l => l.toUpperCase()),
        role
      });
      setIsAuthenticated(true);
      return true;
